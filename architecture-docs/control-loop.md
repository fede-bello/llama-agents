...
- **Retry-exhaustion hook** — The `StepWorkerFailed` branch of `_process_step_result_tick` routes a `StepFailedEvent` to a registered `@catch_error` handler (via `BrokerConfig.catch_error_step_name`). Absent such a handler, or if the handler itself exhausts, the loop emits `WorkflowFailedEvent` and fails the run as before. `FailureInfo` rides on `EventAttempt` between retries and is exposed to step bodies via `Context.retry_info()`.
